...
PAGE_TEXT = {
	PAGE.MAIN: [
		'Сейчас Вы в главном меню. Здесь Вы можете подробнее узнать об игре или '
		'о нас, команде разработчиков. Так же Вы можете попробовать сыграть в '
		'саму игру.',
	],
	PAGE.INFO: [
		'Данный чат-бот был выполнен двумя талантливыми программистами из Лицея '
		'Академии Яндекс в целях защиты проекта WebServer. Вы можете связаться с '
		'нами. Вот наши username '
		'telegram:'
		'\n@av10nyt'
		'\n@name1582222rot3'
	],
	PAGE.HELP: [
		'Lights-out - электронная игра, она содержит поле nxn, каждая клетка '
		'которого может быть в состоянии «включено» или «выключено». Нажатие '
		'на любую клетку поля изменит состояние этой и четырёх соседних клеток.'
		' Цель игры — перевести всё поле в состояние «выключено».'
	...
	PAGE.GAME: [
		'Удачи!',
		'Успехов!',
		'Надеюсь, Вы сможете решить эту непростую головомку!'
	],
}
